...
function isIPRoute(route) {
    if (typeof route.port === undefined) { return false; }
    if (typeof route.port !== 'number') { return false; }
    return true;
}

function isLocalSocketRoute(route) {
    if (typeof route.path === undefined) { return false; }
    if (typeof route.path !== 'string') { return false; }
    return true;
}

function BasicPolicy() {
    if (!(this instanceof BasicPolicy)) { return new BasicPolicy(); }
    ...
    function addIPRoute(route) {
        ...
        if (hosts[fixedhost][route.port] === undefined) { // no routes known for port on the particular host
            internal = {host: route.host, port: route.port}; // avoid side effects and allow lookup
            ...
            self.emit('routeadded',  {port: internal.port, host: internal.host, localAddress: internal.localAddress}); // emit public event (avoid side effects)
        }
    }

    function addLocalSocketRoute(route) {
        if (paths[route.path] === undefined) { // no routes known for the path
            var internal = {path: route.path}; // avoid side effects and allow lookup
            paths[route.path] = internal; // register in the lookup
            self.emit('_routeadded', internal); // emit private event
            self.emit('routeadded',  {path: internal.path}); // emit public event (avoid side effects)
        }
    }
    ...
    function removeIPRoute(route) {
        ...
        if (hosts[fixedhost] !== undefined && hosts[fixedhost][route.port] !== undefined) { // the route is registered
            ...
            self.emit('routeremoved',  {port: internal.port, host: internal.host, localAddress: internal.localAddress}); // emit public event (avoid side effects)
        }
        return self;
    }
    ...
    function removeLocalSocketRoute(route) {
        if (paths[route.path] !== undefined) { // the route is registered
            ...
            self.emit('_routeremoved', internal); // emit private event
            self.emit('routeremoved',  {path: internal.path}); // emit public event (avoid side effect
        }
        return self;
    }
...
util.inherits(BasicPolicy, events.EventEmitter);
