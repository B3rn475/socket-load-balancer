...
function LeastUsedPolicy() {
    if (!(this instanceof LeastUsedPolicy)) { return new LeastUsedPolicy(); }
    ...
    this.manage = function (request) {
        ...
        request.once("completed", function () { // when the connection is closed update the location of the route
            if (route.removed === true) { return; } // the route has been removed, nothing to do
            var queue = queues[route.active]; // get the current queue
...
util.inherits(LeastUsedPolicy, BasicPolicy);

exports.Policy = LeastUsedPolicy;
